...
describe("MochiChef", function () {
  ...
  context("With BEP/LP token added to the field", function () {
    ...
    //   // 30 per block farming rate starting at block 500

    //   this.chef = await this.MochiChef.deploy(this.mochi.address, this.dev.address, "1000000000000000000", "1", "28800")
    //   await this.mochi.addMinter(this.chef.address)
    //   await this.lp.connect(this.alice).approve(this.chef.address, "1000", { from: this.alice.address })
    //   await time.advanceBlockTo("1")
    //   await this.chef.add("1", this.lp.address, true)
    //   await this.chef.connect(this.alice).deposit(0, "1000", { from: this.alice.address })
    ...
    //   await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
    //   expect(await this.chef.pendingMochi(0, this.alice.address)).to.equal("0")
    //   let b = await this.mochi.balanceOf(this.alice.address)
    //   console.log('Alice balance: ' + b);
      
    //   // Block 100
    //   await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
    //   const supply = await this.mochi.totalSupply()
    //   const fSupply = ethers.utils.formatEther(supply)
    //   expect(fSupply).to.equal("82.5")
      
    //   // block 1k
    //   await time.advanceBlockTo("999")
    //   await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
    //   expect(ethers.utils.formatEther(await this.mochi.totalSupply())).to.equal("15290.0")
    
    //   // block 10k
    //   await time.advanceBlockTo("9999")
    //   await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
    //   expect(ethers.utils.formatEther(await this.mochi.totalSupply())).to.equal("262927.5")

    //   // block 50k
    //   await time.advanceBlockTo("49999")
    //   await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
    //   expect(ethers.utils.formatEther(await this.mochi.totalSupply())).to.equal("408837.0")

      // // Block 100k
      // await time.advanceBlockTo("99999")
      // await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
      // expect(ethers.utils.formatEther(await this.mochi.totalSupply())).to.equal("821337.0")

      // // Block 150k
      // await time.advanceBlockTo("149999")
      // await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
      // expect(ethers.utils.formatEther(await this.mochi.totalSupply())).to.equal("1233837.0")

      // await time.advanceBlockTo("199999")
      // await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
      // expect(ethers.utils.formatEther(await this.mochi.totalSupply())).to.equal("1233837.0")
    // })

    // it("should stop giving bonus mochis after the bonus period ends", async function () {
    //   // 1 per block farming rate starting at block 500
    //   this.chef = await this.MochiChef.deploy(this.mochi.address, this.dev.address, "1", "500", "1000")
    ...
    //   expect(await this.chef.pendingMochi(0, this.alice.address)).to.equal("375")
    //   // At block 606, Alice withdraws all pending rewards and should get 10600.
    //   await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
    //   expect(await this.chef.pendingMochi(0, this.alice.address)).to.equal("0")
    //   // expect(await this.mochi.balanceOf(this.alice.address)).to.equal("400")

    //   await time.advanceBlockTo("1000")
    //   await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
    //   expect(await this.chef.pendingMochi(0, this.alice.address)).to.equal("0")
    //   expect(await this.mochi.balanceOf(this.alice.address)).to.equal("10275")
